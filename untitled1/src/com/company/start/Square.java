...
public class Square extends JFrame {
    ...
    public Square() {
        super("Shapes Learning App - Square Descriptions");
        ...
        backButton.addActionListener(new Square.BackButtonActionListener());
        topPanel.add(backButton);

        // Add the title label to the top panel
        JLabel titleLabel = new JLabel("Let's Learn About Square");
        ...
        // Add the description cards for the square
        JPanel SquareCard1 = createDescriptionCard("In Geometry, a square is a two-dimensional plane figure with four equal sides and all the four angles are equal to 90 degrees. The properties of rectangle are somewhat similar to a square, but the difference between the two is, a rectangle has only its opposite sides equal. Therefore, a rectangle is called a square only if all its four sides are of equal length.", "E:\\untitled1\\media\\learnSquare.png");
        shapePanel.add(SquareCard1);

        JPanel SquareCard2 = createDescriptionCard("Looking around, you can find many things that resemble the square shape. Common examples of this shape include a chessboard, craft papers, bread slice, photo frame, pizza box, a wall clock, etc.", "E:\\untitled1\\media\\squareEx.png");
        shapePanel.add(SquareCard2);
    ...
    private class NextButtonActionListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            if (cardIndex < shapePanel.getComponentCount() - 1) {
                ...
                if (cardIndex == shapePanel.getComponentCount() - 1) {
                    nextButton.setText("Let's Create a Square");
                    nextButton.addActionListener(new ActionListener() {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            String buttonText = nextButton.getText();
                            if (buttonText.equals("Let's Create a Square")) {
                                SquareGenerator squareGenerator = new SquareGenerator();
                                squareGenerator.setVisible(true);
                                dispose();
                            }
                        }
                    });
